...
void init_list(GtkWidget *list)
{
    ...
    renderer = gtk_cell_renderer_text_new();
    column = gtk_tree_view_column_new_with_attributes("FG%", renderer, "text", LIST_FPG, NULL);
    gtk_tree_view_append_column(GTK_TREE_VIEW(list), column);
    gtk_tree_view_column_set_clickable(column, TRUE);
    g_signal_connect(column, "clicked", G_CALLBACK(column_clicked), GINT_TO_POINTER(sort_by[2]));

    renderer = gtk_cell_renderer_text_new();
    column = gtk_tree_view_column_new_with_attributes("3P%", renderer, "text", LIST_TPP, NULL);
    gtk_tree_view_append_column(GTK_TREE_VIEW(list), column);
    gtk_tree_view_column_set_clickable(column, TRUE);
